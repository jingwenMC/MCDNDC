...
public class enable extends JCommand {
    @Override
    public boolean onCommand(String[] args, CommandSender sender) {
        if(args.length!=1) return false;
        try {
            boolean isEnabled = Main.getInstance().getExtensionManager().enableExtension(args[0]);
            if(isEnabled)MessageUtil.sendPlayer(sender,"extension.on_enable",new String[]{"%name"},args[0]);
            MessageUtil.sendPlayer(sender,"extension.enabled");
        } catch (Throwable e) {
